...
build: bin/armor get-golint

bin/armor: go-version-checker
	@go install -x
		GOOS=linux GOARCH=386 go build -x -o $(GOBIN)/linux_386/armor
		GOOS=darwin GOARCH=386 go build -x -o $(GOBIN)/darwin_386/armor
#env GOOS=linux CGO_ENABLED=0 go install -v -ldflags $(LD_FLAGS) $(REPO_PATH)
#GOOS=linux CGO_ENABLED=0 go build -o /bin/armor armor.go
#CGO_ENABLED=0 go build -o /bin/armor armor.go
...
test:
	@go test -v -i $(shell go list ./... | grep -v '/vendor\|/pb\|/backup')
	@go test -v $(shell go list ./... | grep -v '/vendor\|/pb\|/backup')

testrace:
	@go test -v -i --race $(shell go list ./... | grep -v '/vendor\|/pb\|/backup')
	@go test -v --race $(shell go list ./... | grep -v '/vendor\|/pb\|/backup')

vet:
	@go vet $(shell go list ./... | grep -v '/vendor\|/pb\|/backup')

fmt:
	@go fmt $(shell go list ./... | grep -v '/vendor\|/pb\|/backup')

lint:
	@for package in $(shell go list ./... | grep -v '/vendor\|/pb\|/backup'); do \
      golint -set_exit_status $$package $$i || exit 1; \
	done

.PHONY: test-cover-html
PACKAGES = $(shell go list ./... | grep -v '/vendor\|/pb\|/backup')
